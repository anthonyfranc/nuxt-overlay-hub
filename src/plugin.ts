...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  const opts = reactive<Required<HubOpts>>({
    ...
    plus: false,
    ...(nuxtApp.$config as any)?.overlayHub
  })

  const track = (c: Component, id: symbol) => {
    let s = reg.value.get(c); if (!s) reg.value.set(c, (s = new Set())); s.add(id)
  }
  const untrack = (c: Component, id: symbol) => {
    const s = reg.value.get(c); if (!s) return; s.delete(id); if (!s.size) reg.value.delete(c)
  }
  ...
  const resolveComp = (input: string) => {
    ...
    return key ? (nuxtApp.vueApp.component(key) as unknown as Component) : (() => null as any)
  }

  function openByComponent<T extends Component>(c: T, props?: any) {
    const inst = core.create(c); track(c, inst.id)
  ...
  if (process.client) {
    ...
    const obs = new MutationObserver(toggleScroll)
