...
export async function applyPlus(hub: any, nuxtApp: any) {
  ...
  const getLatestId = (c: Component) => { const ids = idsOf(c); return ids[ids.length - 1] }

  const pascalize = (t: string[]) =>
    t.filter(Boolean).map(s => s.replace(/[^a-zA-Z0-9]/g,'').replace(/^[a-z]/, c => c.toUpperCase())).join('')
  const normalize = (input: string) =>
    input.includes('/') ? input.split('/').map(s => pascalize(s.split(/[-_]/))).join('')
  ...
  function closeWhere(c: Component, pred: (o:any)=>boolean, value?: any) {
    (hub.overlays || []).forEach((o:any) => { if (o.isOpen && (o as any).component === c && pred(o)) hub.close(o.id, value) })
  }

  // name mirrors
  const openOnceByName = (n: string, key: string, p?: any) => { const c = resolveByName(n); return c ? openOnce(c, key, p) : undefined }
  const openOrFocusByName = (n: string, p?: any) => { const c = resolveByName(n); return c ? openOrFocus(c, p) : undefined }
  const closeAllExceptLatestByName = (n: string, v?: any) => { const c = resolveByName(n); if (c) closeAllExceptLatest(c, v) }
  const waitUntilClosedByName = (n: string, o?: {all?:boolean}) => { const c = resolveByName(n); return c ? waitUntilClosed(c, o) : Promise.resolve() }
  const onChangeByName = (n: string, cb:(i:{ids:symbol[];count:number})=>void) => { const c = resolveByName(n); return c ? onChange(c, cb) : undefined }
  const idsOfByName = (n: string) => { const c = resolveByName(n); return c ? idsOf(c) : [] }
  const countOfByName = (n: string) => idsOfByName(n).length
  const getLatestIdByName = (n: string) => { const ids = idsOfByName(n); return ids[ids.length - 1] }

  Object.assign(hub, {
    idsOf, countOf, getLatestId,
    openOnce, openOrFocus, closeAllExceptLatest, waitUntilClosed, onChange, closeWhere,
